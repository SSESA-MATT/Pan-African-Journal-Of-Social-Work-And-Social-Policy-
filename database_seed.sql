-- Test data for Pan-African Journal Platform
-- Run this in Supabase SQL Editor to create test users and data

-- First, let's create some test users
-- Note: In Supabase, you'll need to create these through the Auth interface first,
-- then update the users table with additional info

-- Insert test users (use actual UUIDs from Supabase Auth)
...
-- Create a test volume and issue
...
INSERT INTO submissions (title, abstract, keywords, author_id, status, manuscript_file_url, submission_date) VALUES
(
...
-- Update the author ID in submissions to use a default value for testing
-- This ensures the API calls will work even without proper auth
UPDATE submissions SET author_id = '00000000-0000-0000-0000-000000000001' WHERE author_id = 'default-author-id';
