-- Journal Enhancement Phase 1 - Complete Database Schema Migration
-- Creates all necessary tables, indexes, and functions for enhanced journal functionality
...
-- Article Metrics Table (aggregated metrics)
...
-- Article Metric Events Table (individual events for detailed analytics)
...
-- Article Keywords Table (normalized keywords for enhanced search)
...
-- Citation Exports Table (tracking citation downloads)
...
-- Related Articles Table (recommendation system)
...
CREATE INDEX IF NOT EXISTS idx_articles_search_composite ON articles(published_at DESC, article_type, language_code) WHERE published_at IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_articles_volume_issue_type ON articles(volume_id, issue_id, article_type);
...
BEGIN
    ...
    SELECT
        ...
        COALESCE(v.volume_number, 1),
        COALESCE(i.issue_number, 1)
    INTO v_year, v_volume, v_issue
    FROM articles a
    LEFT JOIN issues i ON a.issue_id = i.id
    LEFT JOIN volumes v ON (i.volume_id = v.id OR a.volume_id = v.id)
    ...
    LEFT JOIN issues i ON a.issue_id = i.id
    LEFT JOIN volumes v ON (i.volume_id = v.id OR a.volume_id = v.id)
    WHERE COALESCE(v.volume_number, 1) = v_volume AND COALESCE(i.issue_number, 1) = v_issue;

    -- Format DOI string: 10.xxxx/pajswsp.YYYY.VV.II.NNN
    v_doi_string := FORMAT('10.xxxx/pajswsp.%s.%s.%s.%s',
        COALESCE(v_year, EXTRACT(YEAR FROM CURRENT_DATE)),
...
BEGIN
    ...
    IF NEW.keywords IS NOT NULL AND jsonb_typeof(NEW.keywords) = 'array' THEN
    ...
    IF NEW.authors IS NOT NULL AND jsonb_typeof(NEW.authors) = 'array' THEN
...
COMMENT ON TABLE dois IS 'DOI management and registration tracking for articles';
...
COMMENT ON COLUMN articles.volume_id IS 'Reference to the volume this article belongs to';