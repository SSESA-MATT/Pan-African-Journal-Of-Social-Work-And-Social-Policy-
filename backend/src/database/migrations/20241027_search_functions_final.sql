-- Final Search Functions Migration
-- This version handles JSONB arrays properly
...
BEGIN
    ...
    EXCEPTION
        WHEN undefined_column THEN
            ...
            RETURN;
        WHEN OTHERS THEN
            -- Try with TEXT[] array format as fallback
            BEGIN
                RETURN QUERY
                SELECT 
                    unnest_author as author,
                    COUNT(*) as count
                FROM (
                    SELECT unnest(COALESCE(authors::TEXT[], ARRAY[]::TEXT[])) as unnest_author
                    FROM articles
                    WHERE published_at IS NOT NULL
                    AND authors IS NOT NULL
                ) author_list
        ...
        WHEN OTHERS THEN
            ...
            BEGIN
                ...
                SELECT 
                    ...
                    COUNT(*) as count
                FROM (
                    SELECT unnest(COALESCE(keywords::TEXT[], ARRAY[]::TEXT[])) as unnest_keyword
                    FROM articles
                    WHERE published_at IS NOT NULL
                    AND keywords IS NOT NULL
                ) keyword_list
        ...
        WITH filtered_articles AS (
            ...
            WHERE 
                ...
                AND (keyword_filter IS NULL OR (
                    jsonb_to_text_array(a.keywords) && keyword_filter
                ))
...
COMMENT ON FUNCTION get_author_suggestions(TEXT, INTEGER) IS 'Get author name suggestions for autocomplete (JSONB-safe version)';
COMMENT ON FUNCTION get_keyword_suggestions(TEXT, INTEGER) IS 'Get keyword suggestions for autocomplete (JSONB-safe version)';
COMMENT ON FUNCTION get_search_facets(TEXT, TEXT, TEXT[], TIMESTAMP, TIMESTAMP) IS 'Get search facets with JSONB handling (safe version)';
COMMENT ON FUNCTION search_articles_advanced(TEXT, TEXT, TEXT[], INTEGER[], INTEGER[], TIMESTAMP, TIMESTAMP, TEXT[], INTEGER, INTEGER) IS 'Advanced article search with JSONB array support';