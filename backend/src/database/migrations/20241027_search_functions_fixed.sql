-- Fixed Search Functions Migration
-- This version avoids parameter naming conflicts

-- Helper function to check if columns exist (with unique parameter names)
...
BEGIN
    ...
    IF check_column_exists('articles', 'authors') THEN
        RETURN QUERY
        SELECT 
...
BEGIN
    ...
    IF check_column_exists('articles', 'keywords') THEN
        RETURN QUERY
        SELECT 
...
BEGIN
    ...
    has_article_type := check_column_exists('articles', 'article_type');
    has_language_code := check_column_exists('articles', 'language_code');
    ...
    WITH filtered_articles AS (
        ...
        LEFT JOIN issues i ON (has_issues AND check_column_exists('articles', 'issue_id') AND a.issue_id = i.id)
        WHERE 
            a.published_at IS NOT NULL
            AND (search_query IS NULL OR (
                (check_column_exists('articles', 'title') AND a.title ILIKE '%' || search_query || '%') OR
                (check_column_exists('articles', 'abstract') AND a.abstract ILIKE '%' || search_query || '%')
            ))
            AND (author_filter IS NULL OR (
                check_column_exists('articles', 'authors') AND 
            ...
            AND (keyword_filter IS NULL OR (
                check_column_exists('articles', 'keywords') AND COALESCE(a.keywords, ARRAY[]::TEXT[]) && keyword_filter
            ))
...
BEGIN
    ...
    WITH search_results AS (
        SELECT 
            ...
            CASE WHEN check_column_exists('articles', 'authors') THEN COALESCE(a.authors, ARRAY[]::TEXT[]) ELSE ARRAY[]::TEXT[] END as authors,
            CASE WHEN check_column_exists('articles', 'keywords') THEN COALESCE(a.keywords, ARRAY[]::TEXT[]) ELSE ARRAY[]::TEXT[] END as keywords,
            a.published_at,
            a.volume_id,
            CASE WHEN check_column_exists('articles', 'issue_id') THEN a.issue_id ELSE NULL END as issue_id,
            CASE WHEN check_column_exists('articles', 'article_type') THEN COALESCE(a.article_type, 'research_article') ELSE 'research_article' END as article_type,
            CASE WHEN check_column_exists('articles', 'language_code') THEN COALESCE(a.language_code, 'en') ELSE 'en' END as language_code,
            CASE WHEN check_column_exists('articles', 'pdf_url') THEN COALESCE(a.pdf_url, '') ELSE '' END as pdf_url,
            CASE 
        ...
        WHERE 
            ...
            AND (issue_filter IS NULL OR (
                check_column_exists('articles', 'issue_id') AND a.issue_id = ANY(issue_filter)
            ))
            AND (date_from IS NULL OR a.published_at >= date_from)
            AND (date_to IS NULL OR a.published_at <= date_to)
            AND (article_type_filter IS NULL OR (
                check_column_exists('articles', 'article_type') AND 
...
BEGIN
    ...
    IF check_column_exists('articles', 'article_type') THEN
        CREATE INDEX IF NOT EXISTS idx_articles_type ON articles(article_type);
    END IF;
    
    IF check_column_exists('articles', 'language_code') THEN
        CREATE INDEX IF NOT EXISTS idx_articles_language ON articles(language_code);
    END IF;
    
    IF check_column_exists('articles', 'issue_id') THEN
